...
import { isHoneypotFilled, verifyFormToken } from './antispam';
import {
  EMAIL_MAX_LENGTH,
  EMAIL_SHAPE,
  FORM_TOKEN_FIELD,
 ...
 *
 * ── Anti-spam (25/09/2026) ──────────────────────────────────────────────────
 * Deux pièges applicatifs, leurre et jeton horodaté, écartent les robots qui
 * passent par la page. Ils ne bornent PAS le débit : c'est le déclencheur
 * `questions_rate_limit` qui s'en charge, en base, et qui s'applique même à
 * un appel direct à l'API. Ici on traduit seulement son refus.
 */

const UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

/** Message unique des refus non qualifiés : il n'apprend rien à personne. */
const RETRY =
  "Votre question n'a pas pu être envoyée. Réessayez dans un instant.";

/** `program_limit_exceeded` — ce que lève `enforce_question_rate_limit`. */
const RATE_LIMIT_SQLSTATE = '54000';
...
export async function askQuestion(
  ...
  /* Le leurre est invisible et hors parcours de tabulation : un humain ne
     peut pas le remplir. On répond « envoyé » sans rien insérer — dire non
     apprendrait au robot comment s'adapter. */
  ...
  /* Les erreurs de champ passent avant : un visiteur doit d'abord voir ce
     qu'il a à corriger. Le jeton, lui, se juge au moment de l'envoi. */
  const verdict = verifyFormToken(String(formData.get(FORM_TOKEN_FIELD) ?? ''));
  if (verdict === 'perime') {
    return fail(
      {},
      'Cette page est restée ouverte trop longtemps. Rechargez-la, puis renvoyez votre question.',
    );
  }
  if (verdict !== 'ok') return fail({}, RETRY);
  ...
  if (error) {
    /* `54000` : le déclencheur de débit a refusé. Le message reste neutre et
       ne révèle ni le seuil, ni la fenêtre, ni ce qui l'a déclenché. */
    if (error.code === RATE_LIMIT_SQLSTATE) {
      return fail(
        {},
        'Trop de questions ont été envoyées récemment. Réessayez dans un moment.',
      );
    }
    return fail({}, RETRY);
  }
